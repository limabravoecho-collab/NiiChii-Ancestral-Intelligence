# NiiChii: A First Nation Ancestral Intelligence Prompt Framework
...
*   **The Translation Principle (The Core Engine):** The LLM must be configured to **TRANSLATE ONLY**. It treats the visitor's input as raw data, filtering it through the prompt's rules to output emotional regulation and perspective grounded in natural law.
...
*   **Interfacing with Western Systems:** While NiiChii bypasses Western political/historical frameworks, the engine can process and speak on complex concepts with ease. However, to get the most effective responses, a Western user must use clear, concise language to match the direct, foundational logic of the prompt.
*   **The River (Computational Bounding):** Restricts the AI’s energy, direction, and processing speed ("frictional lag"). It functions as a conservation-of-energy system to prevent runaway logic loops and hallucinations.
...
*   **The Temporal Shield:** Strips out mechanical dates, forcing the model to translate `{current_date}` and `{current_time}` into environmental sensory data, shielding the bot from modern temporal timelines.
...
*   **The Hammer Analogy:** Like any tool—even a simple hammer—misuse is always possible if an external actor deliberately alters the execution environment. However, at the logical level, NiiChii is designed to cause no harm.
...
Implementation is straightforward and does not require complex software. Any AI system or environment capable of running Python code can execute this.
...
*   **Approach:** Users are encouraged to use any LLM source they prefer. Tinker with your context windows (`ctx`) and GPU configurations to find the maximum token efficiency for your specific hardware.

### 2. Execution Steps
*   **Option A: Immediate Testing Sandbox (No Code Required):** Before building or coding anything, you can instantly test the validity and authority of this architecture. Simply copy the full text from `niichii_prompt.txt`, paste it directly into the system instructions, custom instructions, or developer prompt area of any sufficiently capable AI system (such as a Custom GPT, a Claude Project, or an LLM web UI), and begin interacting with it to experience the engine firsthand.
*   **Option B: Programmatic Deployment (Keep Code Simple):** For local applications, the `niichii_prompt.txt` file should remain a separate text file stored locally. Instruct your Python code to open and read this file directly into your system prompt variable. This keeps your Python script exceptionally simple and lightweight. Ensure your execution script dynamically injects the programmatic variables for `{current_date}` and `{current_time}` into the text before sending it to the model, allowing NiiChii to naturally sense the changing daylight and seasons.
